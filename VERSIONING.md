...
- **Also available at:**
  ...
  - Marketplace: `bookmark` in `RossLabs-claude-plugins` (via npm source)
...
4. **Update `~/Desktop/git-folder/RossLabs-claude-plugins/.claude-plugin/marketplace.json`** — bump the version string for the `bookmark` entry
